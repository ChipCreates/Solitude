...
import 'move_history_viewer.dart';
...
class GameToolbar extends StatelessWidget {
          ...
          child: Row(
            children: [
              ...
              GameIconButton(
                ...
                onPressed: controller.state == GameState.playing ? () => controller.showHint() : null,
                tooltip: 'Show hint',
                label: 'Hint',
              ),
              const SizedBox(width: 12),
              ...
              Consumer<SettingsProvider>(
                builder: (context, settings, _) {
                  return _buildDifficultyIndicator(context, settings.difficulty);
                },
              ),
              const Spacer(),
  ...
  Widget _buildDifficultyIndicator(BuildContext context, Difficulty difficulty) {
    final brightness = Theme.of(context).brightness;
    Color textColor;
    if (brightness == Brightness.dark) {
      textColor = Colors.white;
    } else {
      textColor = difficulty.color.computeLuminance() < 0.5 ? Colors.white : Colors.black;
    }
